...
extern uchar HMI_RX_BUF[HMID_ID_LEN];
extern uchar HMI_RX_STA;
			...
			while(!outofdata)//没有出现数据异常(即可否找到帧同步字符)
			{
				...
				}else	//找到同步字符了
				{
					...
					{			 
						...
						if(HMI_RX_STA != 0)
						{
							if(HMI_RX_BUF[0] == 0x65)
							{
								hmiid.page = HMI_RX_BUF[1];
								hmiid.id   = HMI_RX_BUF[2];
								hmiid.active = HMI_RX_BUF[3];
							}
							...
							if(hmiid.id == 0x02 || hmiid.id == 0x01)
							{
								audiodev.status &= (0 << 1);
							}
